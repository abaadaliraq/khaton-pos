...
export function SalesReportsDashboard({ variant = "admin" }: { variant?: "admin" | "owner" }) {
  ...
  const loadReport = useCallback(
    async (showLoading = true) => {
      ...
      try {
        ...
        const nextTodaySummary =
          !isOwner || (selectedRange.from === today && selectedRange.to === today)
            ? nextReport.summary
            : (await getAdminSalesReport({ from: today, to: today, today })).summary;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status, error) => {
        if (error) {
          console.error("Sales reports realtime subscription error", error);
        }

        if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
          console.error("Sales reports realtime subscription failed", { status });
        }
      });
    ...
    <div className="space-y-5">
      <section className="rounded-md border border-[#e4d8c8] bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-sm text-[#a65f3f]">{isOwner ? "لوحة المالك" : "Sales & Orders Reports"}</p>
        ...
        <>
          {isOwner ? (
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              <KpiCard title="إجمالي المبيعات" value={formatCurrency(summary.revenue)} helper="للفترة المختارة" icon={ReceiptText} current={summary.revenue} previous={previousSummary.revenue} />
              <KpiCard title="عدد الطلبات" value={formatNumber(summary.orderCount) + " طلب"} helper="طلبات لديها دفعة مكتملة" icon={BarChart3} current={summary.orderCount} previous={previousSummary.orderCount} />
              ...
              <KpiCard title="مبيعات اليوم" value={formatCurrency(todaySummary.revenue)} helper="حسب يوم بغداد" icon={ReceiptText} current={todaySummary.revenue} previous={0} />
              <KpiCard title="مبيعات الأسبوع" value={formatCurrency(report.week.summary.revenue)} helper="مقارنة بالأسبوع السابق" icon={BarChart3} current={report.week.summary.revenue} previous={report.week.previousSummary.revenue} />
              <KpiCard title="مبيعات الشهر" value={formatCurrency(report.month.summary.revenue)} helper="مقارنة بالشهر السابق" icon={TrendingUp} current={report.month.summary.revenue} previous={report.month.previousSummary.revenue} />
            </section>
          ) : (
