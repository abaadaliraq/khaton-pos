...
create policy "cash shifts role read" on public.cash_shifts
  ...
  using (
    public.current_user_role() in ('admin', 'accountant', 'owner')
    or (
...
begin
  if requester_role not in ('cashier', 'accountant', 'admin', 'owner') then
    raise exception 'Only cashier, accountant, admin, or owner users can open cash shifts';
  end if;
...
begin
  if requester_role not in ('cashier', 'accountant', 'admin', 'owner') then
    raise exception 'Only cashier, accountant, admin, or owner users can close cash shifts';
  end if;
...
create policy "cash movements finance read" on public.cash_movements
  for select to authenticated
  using (
    public.current_user_role() in ('accountant', 'admin', 'owner')
    or (
      public.current_user_role() = 'cashier'
