create table if not exists public.inventory_waste_reports (
  id uuid primary key default gen_random_uuid(),
  report_number bigint generated always as identity unique,
  context text not null check (context in ('warehouse', 'issued_department')),
  destination text check (destination in ('kitchen', 'barista', 'bar', 'service', 'cleaning', 'management', 'other')),
  ...
  constraint inventory_waste_reports_context_destination_check
    check (
      (context = 'warehouse' and destination is null)
      or (context = 'issued_department' and destination is not null)
    )
);

create table if not exists public.inventory_waste_items (
  id uuid primary key default gen_random_uuid(),
  report_id uuid not null references public.inventory_waste_reports(id) on delete restrict,
  inventory_item_id uuid not null references public.inventory_items(id) on delete restrict,
  quantity numeric(18,3) not null check (quantity > 0),
  unit_id uuid references public.inventory_units(id) on delete restrict,
  conversion_id uuid references public.inventory_item_conversions(id) on delete restrict,
  unit_label text not null check (length(btrim(unit_label)) > 0),
  quantity_base numeric(18,3) not null check (quantity_base > 0),
  reason text not null check (reason in ('expired', 'spoiled', 'damaged', 'contaminated', 'broken', 'preparation_error', 'overproduction', 'oil_disposal', 'other')),
  notes text,
  requisition_id uuid references public.inventory_requisitions(id) on delete restrict,
  requisition_item_id uuid references public.inventory_requisition_items(id) on delete restrict,
  created_at timestamptz not null default now(),
  constraint inventory_waste_items_unit_check
    check (
      (unit_id is not null and conversion_id is null)
      or (unit_id is null and conversion_id is not null)
    )
);

create index if not exists inventory_waste_reports_context_idx on public.inventory_waste_reports(context);
create index if not exists inventory_waste_reports_destination_idx on public.inventory_waste_reports(destination);
create index if not exists inventory_waste_reports_posted_at_idx on public.inventory_waste_reports(posted_at desc);
create index if not exists inventory_waste_items_report_id_idx on public.inventory_waste_items(report_id);
create index if not exists inventory_waste_items_inventory_item_id_idx on public.inventory_waste_items(inventory_item_id);
create index if not exists inventory_waste_items_requisition_item_id_idx on public.inventory_waste_items(requisition_item_id);

alter table public.inventory_movements
  add column if not exists inventory_waste_report_id uuid references public.inventory_waste_reports(id) on delete set null,
  add column if not exists inventory_waste_item_id uuid references public.inventory_waste_items(id) on delete set null;

create index if not exists inventory_movements_waste_report_id_idx on public.inventory_movements(inventory_waste_report_id);
create index if not exists inventory_movements_waste_item_id_idx on public.inventory_movements(inventory_waste_item_id);

create or replace function public.prevent_inventory_waste_mutation()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  raise exception 'Posted inventory waste records are immutable';
end;
$$;

drop trigger if exists prevent_inventory_waste_reports_update on public.inventory_waste_reports;
create trigger prevent_inventory_waste_reports_update before update on public.inventory_waste_reports
  for each row execute function public.prevent_inventory_waste_mutation();

drop trigger if exists prevent_inventory_waste_reports_delete on public.inventory_waste_reports;
create trigger prevent_inventory_waste_reports_delete before delete on public.inventory_waste_reports
  for each row execute function public.prevent_inventory_waste_mutation();

drop trigger if exists prevent_inventory_waste_items_update on public.inventory_waste_items;
create trigger prevent_inventory_waste_items_update before update on public.inventory_waste_items
  for each row execute function public.prevent_inventory_waste_mutation();

drop trigger if exists prevent_inventory_waste_items_delete on public.inventory_waste_items;
create trigger prevent_inventory_waste_items_delete before delete on public.inventory_waste_items
  for each row execute function public.prevent_inventory_waste_mutation();
...
create or replace function public.resolve_inventory_waste_quantity(
...
begin
  ...
  if p_conversion_id is not null then
    ...
    quantity_base := round(p_quantity * target_conversion.quantity_in_base_unit, 3);
    unit_label := target_conversion.packaging_unit_name_ar;
  else
    ...
    unit_label := target_unit.name_ar;
  end if;

  return jsonb_build_object('quantity_base', quantity_base, 'unit_label', unit_label);
end;
$$;
...
begin
  ...
  if p_context = 'issued_department' and requester_role not in ('admin', 'storekeeper', 'kitchen') then
...
alter table public.inventory_waste_reports enable row level security;
alter table public.inventory_waste_items enable row level security;
...
create policy "inventory waste reports role read"
  ...
  to authenticated
  using (
    public.current_user_role() in ('admin', 'storekeeper', 'accountant')
...
grant select on public.inventory_waste_reports to authenticated;
grant select on public.inventory_waste_items to authenticated;

revoke all on function public.prevent_inventory_waste_mutation() from public;
revoke all on function public.waste_destination_allowed(text) from public;
revoke all on function public.resolve_inventory_waste_quantity(uuid, numeric, uuid, uuid) from public;
  ...
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    if not exists (
      ...
      where pubname = 'supabase_realtime'
        ...
        and tablename = 'inventory_waste_reports'
    ) then
      alter publication supabase_realtime add table public.inventory_waste_reports;
    end if;
    ...
    if not exists (
      ...
      where pubname = 'supabase_realtime'
        and schemaname = 'public'
        and tablename = 'inventory_waste_items'
    ) then
      alter publication supabase_realtime add table public.inventory_waste_items;
    end if;
  end if;
end $$;
