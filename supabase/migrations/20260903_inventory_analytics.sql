create index if not exists purchase_items_inventory_item_created_idx
  on public.purchase_items(inventory_item_id, created_at desc);

create index if not exists inventory_requisition_items_item_status_idx
  on public.inventory_requisition_items(inventory_item_id, status);

create index if not exists inventory_waste_items_item_created_idx
  on public.inventory_waste_items(inventory_item_id, created_at desc);

create index if not exists orders_status_opened_at_idx
  on public.orders(status, opened_at desc);
...
declare
  ...
  payload jsonb;
begin
  if requester_role not in ('admin', 'storekeeper', 'accountant') then
